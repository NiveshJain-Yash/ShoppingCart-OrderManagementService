...
describe('CouponInput Component (FR-13)', () => {
  ...
  it('should show success message when valid coupon applied', async () => {
    const mockApply = vi.fn().mockImplementation(() => Promise.resolve());
  ...
  it('should display applied coupon message', () => {
    ...
    expect(screen.getByText(/Coupon "SAVE50" is applied/)).toBeInTheDocument();
  });
  ...
  it('should show error message when coupon application fails', () => {
    render(
      <CouponInput
        ...
        isLoading={false}
        appliedCoupon={null}
        error="Invalid or unknown coupon"
      />
    );
